...
class FeatureExtractor:
    ...
    def extract_features_from_file(self, songname):
    ...
    def process_file(self, filename):
        ...
        # Predict with the model if it's available
        if self.model:
            ...
            features['class'] = [k for k, v in self.emotion_map.items() if v == predicted_label][0]
    ...
    def update_database(self):
        if os.path.exists(self.database_csv):
            ...
            existingdata_df = pd.DataFrame(columns=[
                'id', 'song_name', 'class', 'label', 'tempo', # Add other feature columns
            ])
            ...
            for filename in file_names:
                if filename.endswith(('.wav', '.mp3')):
                    if 'song_name' in existingdata_df.columns and filename in existingdata_df['song_name'].values:
                        continue
                    executor.submit(self._process_and_append, filename, new_files_data, id_count)
                    id_count += 1
    ...
    def _process_and_append(self, filename, new_files_data, id_count):
